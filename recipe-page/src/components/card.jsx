const Card = ({ recipe }) => {
  ...
  return (
    <div className={"flex justify-center min-h-screen md:py-10"}>
      <article className={"w-mobile md:w-desktop"} aria-labelledby={"title"}>
        <img className={"m-0 object-cover p-0 block md:hidden md:rounded-lg"} src={image} alt={title} />
        <div className={"bg-white text-left text-stone-900 p-8 md:rounded-2xl md:p-10"}>
          <img className={"object-cover hidden md:block md:rounded-lg"} src={image} alt={title} />
          <h1 className={"font-young text-4xl mt-4 md:mt-10"} id={"title"}>{title}</h1>
