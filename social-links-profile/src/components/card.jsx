const Card = ({ info }) => {
  ...
  return (
    <article className={"absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-gray-800 text-center sm:w-[326px] sm:pb-6 md:w-[384] md:pb-10"} aria-labelledby={"user-name"}>
      ...
      <h1 className="text-[22px] font-bold sm:mt-6 md:mt-8" id={"user-name"}>
        {name}
      </h1>
      ...
      <section className="sm:px-6 md:px-10">
        <ul className="flex flex-col items-center space-y-4">
          {links.map((link, index) => (
            <li key={index} className="w-full">
              <a href={link.url} target={"_blank"} className="block w-full rounded-lg bg-gray-700 px-10 py-3 text-center text-[15px] font-bold hover:bg-green-social hover:text-gray-800">
                {link.name}
              </a>
            </li>
          ))}
