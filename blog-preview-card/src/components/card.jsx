...
const Card = ({ info }) => {
  ...
  return (
    <div className={"flex items-center justify-center h-screen"}>
      <article className={"rounded-2xl border border-black bg-white shadow-[9px_9px_0px_black] sm:w-[326px] md:w-[380]"} aria-labelledby={"card-title"}>
        ...
        <div className={"justify-left flex px-[22px] pt-1"}>
          <p
            className="inline-block rounded bg-[#f5d04e] px-3 py-1 text-left font-bold text-gray-900 sm:text-[14px] md:text-[16px]">{category}</p>
        </div>
        <p className={"px-[22px] pt-3 text-left text-gray-900 sm:text-[12px] md:text-[14px]"}>Published {publishedDate}</p>
        <h1
          className={"cursor-pointer px-[22px] pt-3 text-left font-bold text-gray-900 hover:text-[#f5d04e] sm:text-[20px] md:text-[24px]"}
          id={"card-title"}>
          {title}
        </h1>
